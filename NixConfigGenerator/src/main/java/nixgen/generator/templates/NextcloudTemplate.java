package nixgen.generator.templates;

import nixgen.model.ConfigData;

public class NextcloudTemplate {
    public static String generate(ConfigData config) {
        return String.format("""
{ config, lib, pkgs, ... }:

{
  services.nextcloud = {
    enable = true;
    package = pkgs.nextcloud33;
    hostName = "%s";
    https = true;
    datadir = "/srv/data/nextcloud";

    settings = {
      trusted_proxies = [ "127.0.0.1" ];
      trusted_domains = [ "%s" ];
      overwriteprotocol = "https";
      default_phone_region = "%s";
      maintenance_window_start = 1;
    };

    config = {
      adminuser = "%s";
      adminpassFile = config.sops.secrets."nextcloud/admin_password".path;

      dbtype = "pgsql";
      dbname = "nextcloud";
      dbuser = "nextcloud";
      dbhost = "/run/postgresql";
    };

    database.createLocally = true;

    configureRedis = true;
    caching.redis = true;

    phpOptions = {
      "opcache.interned_strings_buffer" = "16";
      "opcache.memory_consumption" = "256";
    };

    maxUploadSize = "16G";
  };

  # Make Nextcloud listen on 8080 for Caddy to proxy
  services.nginx.virtualHosts."%s" = {
    listen = [{
      addr = "127.0.0.1";
      port = 8080;
    }];
  };

  services.postgresql = {
    enable = true;
    ensureDatabases = [ "nextcloud" ];
    ensureUsers = [{
      name = "nextcloud";
      ensureDBOwnership = true;
    }];
  };

  systemd.services."nextcloud-setup" = {
    requires = [ "postgresql.service" "srv-data.mount" ];
    after = [ "postgresql.service" "srv-data.mount" ];
  };
}
""", config.getNasDomain(), config.getNasDomain(), config.getPhoneRegion(),
     config.getAdminUsername(), config.getNasDomain());
    }
}
